...
class VerkleProofGenerator(BaseProver):
    ...
    def generate_proof(self, tree: VerkleTree, keys: list[bytes]):
        ...
        r = derive_r(generate_root_bytes(committee_root), [tree._key_to_path(k) for k in keys], b.curve_order)
        #print("r", r)
        ...
        for key in keys:
            ...
            for level, idx in enumerate(path):
                ...
                r_factor = derive_r_factor_hash(generate_root_bytes(committee_root), path, level, b.curve_order)
