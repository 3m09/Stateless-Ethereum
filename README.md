...
secured account keys are routed through each structure.
...
| Web prover/verifier runner | Planned | Existing proof registries are not connected to the web app yet |
...
between 1 and 250 keys, limited further by the number of accounts in its source
dataset.
...
| `STATELESS_ETHEREUM_MIN_REQUEST_INTERVAL_SECONDS` | `0.05` | Per-request pacing |
...
- Accept an explicit list of up to 250 addresses.
- Scan up to 128 recent blocks for transaction participants.
...
- The pinned block number, hash, timestamp, state root, and chain ID.
...
configuration files and toolchain assumptions. Some legacy JSON files still
contain setup values. Do not serve those files publicly. The web application
does not load them or expose their contents.
...
The exact proof compatibility matrix is in
[`ARCHITECTURE.md`](ARCHITECTURE.md). The proof code is experimental and is not
yet supervised by the web job system.
...
secret redaction, persistence, manifests, and visualization data.

## Operational limitations

- Import and tree jobs currently run as in-process asynchronous tasks.
- Run one Uvicorn worker so jobs are not split across independent process
  memory. A durable external queue is planned.
- The demonstrator currently imports at most 250 accounts per dataset.
