...
class MerkleProofVerifier(BaseVerifier):
    ...
    def verify_proof(
        ...
        if witness != root_hash:
            ...
            return False
        if len(proof_paths) != len(keys) or len(keys) != len(values):
            return False
        ...
        for key, expected_value in zip(keys, values, strict=True):
    ...
    def _verify_single_proof(
        ...
        # Preserve the tree's configured radix rather than assuming the
        # Ethereum-canonical width of 16.
        try:
